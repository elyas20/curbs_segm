...
from loss import multiclass_dice_coeff, dice_coeff, dice_loss, FocalLoss

val_criterion = nn.CrossEntropyLoss()
val_loss_mse = nn.MSELoss(reduction='sum')
...
def evaluate(net, dataloader, device):
    ...
    for batch in tqdm(dataloader, total=num_val_batches, desc='Validation round', unit='batch', leave=False):
        ...
        with torch.no_grad():
            # predict the mask
            mask_pred = net(image)
            # loss = val_loss_mse(mask_pred, mask_true)
            loss = val_criterion(mask_pred, mask_true) \
                    + dice_loss(F.softmax(mask_pred, dim=1).float(),
                                F.one_hot(mask_true, net.n_classes).permute(0, 3, 1, 2).float(),
                                multiclass=True)
            # loss = focal_crit(mask_pred, mask_true)
